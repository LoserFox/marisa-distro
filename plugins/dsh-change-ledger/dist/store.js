...
export class LedgerStore {
    ...
    async initialize() {
        ...
        for (const workspaceKey of await safeDirectoryNames(join(this.config.storageDir, 'workspaces'))) {
            ...
            if (await this.workspaceAppearsActive(workspaceDir))
                continue;
    ...
    async acquire(workspace) {
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
            ...
            const record = { pid: process.pid, createdAt: Date.now(), nonce };
            try {
    ...
    async collectGarbage(workspace, additionalReferenced = []) {
    ...
    async reclaimStaleLock(lockPath) {
        ...
        if (processExists(lock.pid))
            return false;
        if (Date.now() - lock.createdAt < this.config.staleLockMs)
            return false;
        try {
            const current = parseLock(await readJson(lockPath));
    ...
    async workspaceAppearsActive(workspaceDir) {
        ...
        try {
            ...
            if (error instanceof ChangeLedgerError && error.code === 'STATE_READ_FAILED' && isMissingCause(error.cause))
                return false;
            if (isNodeError(error, 'ENOENT'))
                return false;
            try {
...
function parseLock(value) {
    ...
    return { pid: record.pid, createdAt: record.createdAt, nonce: record.nonce };
}
