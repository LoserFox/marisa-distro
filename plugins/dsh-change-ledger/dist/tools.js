        ...
        const disposers = [
            ...
            harness.tools.register({
                ...
                async execute(args, exec) {
                    ...
                    if (limit > MAX_TOOL_PAGE)
                        throw new ChangeLedgerError('INVALID_ARGUMENTS', `limit must not exceed ${MAX_TOOL_PAGE}`);
                    return {
            ...
            harness.tools.register({
                ...
                async execute(args, exec) {
                    ...
                    if (limit > MAX_RECOVERY_PAGE)
                        throw new ChangeLedgerError('INVALID_ARGUMENTS', `limit must not exceed ${MAX_RECOVERY_PAGE}`);
                    return {
...
function executionWorkspace(exec) {
    ...
    if (agent === undefined)
        throw new ChangeLedgerError('AGENT_REQUIRED', 'change-ledger tools require a calling DSH agent');
    const cwd = agent.session.header.cwd;
    if (cwd === undefined)
        throw new ChangeLedgerError('WORKSPACE_REQUIRED', 'the calling session has no working directory');
...
function optionalBoolean(value, name) {
    ...
    if (typeof value !== 'boolean')
        throw new ChangeLedgerError('INVALID_ARGUMENTS', `${name} must be a boolean`);
    return value;
}
function optionalStringArray(value, name) {
    ...
    if (!Array.isArray(value))
        throw new ChangeLedgerError('INVALID_ARGUMENTS', `${name} must be an array`);
...
function simplifyChange(change) {
    return {
        path: change.path,
        kind: change.kind,
...
function truncateText(value) {
    if (value.length <= MAX_DIAGNOSTIC_TEXT)
        return value;
