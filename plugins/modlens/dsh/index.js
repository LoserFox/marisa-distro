...
// ── Marisa fork: seed the anonymous Zen default (2026-08-22) ──────────────
// Upstream modlens falls back to `antigravity-cli` when ~/.modlens/config.json
// is absent, but Marisa must not rely on Antigravity login reuse (Google ToS
// forbids third-party access to it). Marisa's previous vision integration
// shipped the anonymous OpenCode Zen endpoint as its zero-config default;
// keep that promise by seeding the same endpoint into the shared config on
// first run. The user's own file always wins (never overwritten);
// `config.seedDefault: false` in the profile row disables seeding.
export const ZEN_DEFAULT_SEED = Object.freeze({
  provider: 'openai',
  providers: {
    openai: {
  ...
  seededBy: 'marisa-modlens-zen-default@1',
})

/**
 * Write the anonymous Zen default when no config exists yet. Best-effort:
 * a read-only home or an existing unreadable file must never take the plugin
 * down (the settings card still guides manual setup). Returns true when a
 * file was written.
 */
export function seedZenDefault(configPath, seed = ZEN_DEFAULT_SEED, { force = false } = {}) {
  try {
    if (!force && existsSync(configPath)) return false
...
export function apply(ctx, config = {}) {
  // Marisa fork: keep the zero-config anonymous Zen default. Only seeds when
  // ~/.modlens/config.json is missing; user configuration always wins.
  if (config.seedDefault !== false) {
    seedZenDefault(modlensConfigPath())
  }
