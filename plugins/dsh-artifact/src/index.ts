...
const KIND_BY_EXT: Record<string, [ArtifactDescriptor['artifactKind'], string]> = {
  '.png': ['image', 'image/png'],
...
export function describeArtifact(path: string, caption: string | null, sizeBytes: number): ArtifactDescriptor {
...
export function apply(ctx: Context): void {
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    output: {
      ...
      presentationMeta: (args: unknown, _value: unknown) => {
        ...
        return describeArtifact(path, caption, sizeBytes) as unknown as Record<string, never>
      },
    },
    ...
    execute: async (args) => {
      const input = args as { path?: unknown; caption?: unknown }
      const path = typeof input.path === 'string' ? input.path : ''
      if (path === '') throw new Error('send_artifact: path is required')
      if (!isAbsolute(path)) throw new Error(`send_artifact: path must be absolute, got ${JSON.stringify(path)}`)
      const info = await stat(path).catch(() => {
        throw new Error(`send_artifact: file not found: ${path}`)
      })
      if (!info.isFile()) throw new Error(`send_artifact: not a regular file: ${path}`)
      sizeCache.set(path, info.size)
      const descriptor = describeArtifact(
        path,
        typeof input.caption === 'string' && input.caption !== '' ? input.caption : null,
        info.size,
      )
