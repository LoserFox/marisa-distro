...
export function parseProxy(raw: string): ProxyConfig {
  ...
  return {
    scheme: scheme as ProxyConfig['scheme'],
    host: u.hostname,
...
export function displayProxy(proxy: ProxyConfig): string {
...
export const DEFAULT_NO_PROXY: readonly string[] = ['localhost', '127.0.0.1', '::1']
 ...
 */
export function inNoProxy(host: string, port: number, extra: readonly string[] = []): boolean {
...
class HandshakeReader {
  ...
  private onData = (chunk: Buffer): void => {
    ...
    if (!this.waiter || this.buffer.length < this.waiter.n) return
...
function socks5Connect(proxy: ProxyConfig, host: string, port: number): Promise<net.Socket> {
  return new Promise((resolve, reject) => {
...
function readHeadersBlock(sock: net.Socket): Promise<string> {
  return new Promise((resolve, reject) => {
    const chunks: Buffer[] = []
    let size = 0
    let settled = false
    ...
    const onData = (chunk: Buffer): void => {
      chunks.push(chunk)
      size += chunk.length
      if (size > 8192) return fail(new Error('proxy handshake response too large'))
      const idx = Buffer.concat(chunks).indexOf('\r\n\r\n')
      if (idx >= 0) done(Buffer.concat(chunks).toString('latin1', 0, idx))
    }
    const onError = (err: Error): void => fail(err)
    const onEnd = (): void => fail(new Error('proxy closed during handshake'))
    sock.on('data', onData)
...
function httpProxyConnect(proxy: ProxyConfig, host: string, port: number): Promise<net.Socket> {
  return new Promise((resolve, reject) => {
 ...
 */
export function connectSocket(proxy: ProxyConfig, host: string, port: number, noProxyExtra: readonly string[] = []): Promise<net.Socket> {
