...
type ProviderPreset = 'zen' | 'glm' | 'custom'

function providerPresetOf(draft: Pick<Draft, 'baseUrl' | 'model' | 'authMode'>): ProviderPreset {
  if (
    draft.baseUrl === 'https://opencode.ai/zen/v1'
...
function selectProviderPreset(draft: Draft, preset: ProviderPreset): Draft {
  if (preset === 'zen') {
    return {
      ...draft,
      providerPreset: 'zen',
...
function draftOf(value: SettingsValue): Draft {
  const draft: Omit<Draft, 'providerPreset'> = {
...
function healthDetail(name: string, detail: string, t: Translate): string {
  ...
  if (detail === 'Anonymous provider; no user credential is required') return t('healthCredentialAnonymous')
...
function LoadedSettings({ controller, t }: SettingsInjected) {
  ...
  return (
    <div className="dvt-settings">
      ...
      <section className="dvt-panel dvt-essential"><div className="dvt-panel-title"><div><h3>{t('provider')}</h3><p>{t('providerHint')}</p></div><span className={`dvt-badge ${draft.authMode === 'none' || snapshot.credential.configured ? 'ok' : 'error'}`}>{draft.authMode === 'none' ? t('anonymous') : snapshot.credential.configured ? t('configured') : t('missing')}</span></div>
        ...
        <div className="dvt-form-grid">
          <Field label={t('providerPreset')}><select disabled={!snapshot.writable || busy} value={draft.providerPreset} onChange={(event) => { setDraft(current => current === undefined ? current : selectProviderPreset(current, event.target.value as ProviderPreset)); if (event.target.value === 'zen') setApiKey('') }}><option value="zen">{t('presetZen')}</option><option value="glm">{t('presetGlm')}</option><option value="custom">{t('presetCustom')}</option></select></Field>
          {draft.providerPreset === 'custom' ? <Field label={t('authMode')}><select disabled={!snapshot.writable || busy} value={draft.authMode} onChange={(event) => { update('authMode', event.target.value as 'none' | 'credential'); if (event.target.value === 'none') setApiKey('') }}><option value="none">{t('anonymous')}</option><option value="credential">{t('credentialAuth')}</option></select></Field> : null}
          ...
          {draft.authMode === 'credential' ? <Field label={t('apiKey')} hint={keyLocked ? t('apiKeyLocked') : snapshot.credential.source === undefined ? t('apiKeyHint') : `${t('apiKeyHint')} ${t('sourceHint', { source: t('source'), value: credentialSource(snapshot.credential.source, t) })}`}><Input aria-label={t('apiKey')} type="password" autoComplete="new-password" disabled={busy || keyLocked} placeholder={snapshot.credential.configured ? t('apiKeyPlaceholderConfigured') : t('apiKeyPlaceholderMissing')} value={apiKey} onChange={(event) => { setApiKey(event.target.value); setDraftError(undefined) }} /></Field> : null}
        </div>
        {draft.providerPreset === 'zen' ? <div className="dvt-provider-guide">{t('zenGuide')}</div> : null}
        {draft.providerPreset === 'glm' ? <div className="dvt-provider-guide"><span>{t('glmGuide')}</span><nav><a href="https://bigmodel.cn/console/overview" target="_blank" rel="noreferrer">{t('registerGlm')}</a><a href="https://bigmodel.cn/usercenter/proj-mgmt/apikeys" target="_blank" rel="noreferrer">{t('createGlmKey')}</a></nav></div> : null}
      </section>
