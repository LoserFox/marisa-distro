...
function LoadedSettings({ controller, t }: SettingsInjected) {
  ...
  const update = <K extends keyof Draft>(key: K, value: Draft[K]): void => setDraft(current => {
    if (current === undefined) return current
    const isProviderField = key === 'baseUrl' || key === 'authMode' || key === 'credential' || key === 'model'
    return { ...current, ...(isProviderField ? { providerPreset: 'custom' as const } : {}), [key]: value }
  })
  const save = (): void => {
    try {
  ...
  return (
    <div className="dvt-settings">
      ...
      <section className="dvt-panel"><div className="dvt-panel-title"><h3>{t('provider')}</h3><span className={`dvt-badge ${draft.authMode === 'none' || snapshot.credential.configured ? 'ok' : 'error'}`}>{draft.authMode === 'none' ? t('anonymous') : snapshot.credential.configured ? t('configured') : t('missing')}</span></div>
        <div className="dvt-form-grid">
          <Field label={t('providerPreset')}><select value={providerPreset} onChange={(event) => { setDraft(current => current === undefined ? current : selectProviderPreset(current, event.target.value as ProviderPreset)) }}><option value="zen">{t('presetZen')}</option><option value="glm">{t('presetGlm')}</option><option value="custom">{t('presetCustom')}</option></select></Field>
          {providerPreset === 'custom' ? <Field label={t('authMode')}><select value={draft.authMode} onChange={(event) => { update('authMode', event.target.value as 'none' | 'credential') }}><option value="none">{t('anonymous')}</option><option value="credential">{t('credentialAuth')}</option></select></Field> : null}
          ...
          {draft.authMode === 'credential' ? <Field label={t('credential')} hint={snapshot.credential.source === undefined ? undefined : `${t('source')}: ${snapshot.credential.source}`}><Input value={draft.credential} onChange={(event) => { update('credential', event.target.value) }} /></Field> : null}
        ...
        {providerPreset === 'zen' ? <div className="dvt-provider-guide">{t('zenGuide')}</div> : null}
        {providerPreset === 'glm' ? <div className="dvt-provider-guide"><span>{t('glmGuide')}</span><nav><a href="https://bigmodel.cn/console/overview" target="_blank" rel="noreferrer">{t('registerGlm')}</a><a href="https://bigmodel.cn/usercenter/proj-mgmt/apikeys" target="_blank" rel="noreferrer">{t('createGlmKey')}</a></nav></div> : null}
      </section>
