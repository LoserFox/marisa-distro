...
const en = {
    ...
    testing: 'Checking…',
    connectionHint: 'Connection testing sends GET /models without an image or completion. Credential mode includes the selected DSH Credential; anonymous mode sends no user credential.',
...
const zh = {
    ...
    testConnection: '测试连接',
    testing: '检查中…',
    connectionHint: '连接测试只请求 GET /models，不上传图片或创建 completion。Credential 模式会发送所选 DSH Credential；匿名模式不发送用户凭据。',
    pluginVersion: '插件',
    upstreamVersion: '上游',
...
function LoadedSettings({ controller, t }) {
    ...
    const update = (key, value) => setDraft(current => {
        if (current === undefined)
            return current;
        const isProviderField = key === 'baseUrl' || key === 'authMode' || key === 'credential' || key === 'model';
        return { ...current, ...(isProviderField ? { providerPreset: 'custom' } : {}), [key]: value };
    });
    const save = () => {
        try {
...
.dvt-provider-guide{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;padding-top:2px;color:var(--dsw-alias-fg-muted,#77736d);font-size:11px;line-height:1.5}.dvt-provider-guide nav{display:flex;gap:10px;flex:none}.dvt-provider-guide a{color:#6758d4;font-weight:600;text-decoration:none}.dvt-provider-guide a:hover{text-decoration:underline}
...
@media(max-width:720px){.dvt-settings-header{display:grid}.dvt-release{width:auto}.dvt-form-grid{grid-template-columns:1fr}.dvt-metrics{grid-template-columns:1fr}.dvt-artifact-meta{align-items:flex-start;flex-direction:column}.dvt-panel-title,.dvt-provider-guide{flex-direction:column}}
`;
