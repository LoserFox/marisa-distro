...
async function profileFixture(spec = '0.1.0') {
  ...
  const installedDir = join(profileDir, 'node_modules', '@dsh-external', 'dsh-vision-toolkit')
...
describe('plugin restart helper', () => {
  it.skipIf(process.platform === 'win32')('rolls back and restores service when the replacement exits before becoming ready', async () => {
    ...
    const installedPackagePath = join(root, 'node_modules', '@dsh-external', 'dsh-vision-toolkit', 'package.json')
...
const target = process.argv.find(value => value.startsWith('@dsh-external/dsh-vision-toolkit@'))
if (!target) process.exit(1)
const version = target.slice(target.lastIndexOf('@') + 1)
writeFileSync(process.env.DVT_RESTART_STATE, version)
writeFileSync(process.env.DVT_INSTALLED_PACKAGE, JSON.stringify({ name: '@dsh-external/dsh-vision-toolkit', version }))
`)
