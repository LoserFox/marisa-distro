...
export function terminalArgv(shell, paths, distro) {
  switch (shell) {
    ...
    case "msys2": return [paths.msys2, "-i"];
...
export function terminalTool(ctx, registry, defaultsFn) {
  return defineTool({
    ...
    parameters: {
      ...
      shell: {
        type: "string",
        enum: ["powershell", "msys2", "gitbash", "wsl"],
        description: "With action open: terminal backend for the session. Defaults to the user's configured default terminal. Use only when the user asks to run in a specific shell."
      },
      distro: {
        type: "string",
        description: "WSL distribution (only when the shell is wsl). Defaults to the configured wslDistro, then the system default."
      },
      input: {
        type: "string",
    ...
    async execute(args, exec) {
      ...
      switch (args.action) {
        case "open": {
          ...
          const shell = args.shell ?? defaults.defaultShell;
          if (["powershell", "msys2", "gitbash", "wsl"].indexOf(shell) === -1) {
            throw new Error("terminal: invalid shell " + JSON.stringify(shell));
          }
          const argv = terminalArgv(shell, defaults.paths, args.distro ?? defaults.wslDistro);
          ...
          if (shell === "msys2" && (typeof env.MSYSTEM !== "string" || env.MSYSTEM.length === 0)) {
            env.MSYSTEM = defaults.msys2Msystem ?? "MINGW64";
          }
