...
> **Marisa fork（v0.4.0）**：本目录是 Marisa 发行版维护的 fork（基线 npm `dsh-bash-terminal@0.3.14`，MIT）。与上游差异：
>
> 1. **新增 `msys2` 后端**：MSYS2 根目录 `bash -lc`（自动探测 `C:\msys64` 等 + `msys2Root` 配置，注入 `MSYSTEM=MINGW64`），pacman/mingw 工具链可用；
> 2. **按调用切换**：`shell` / `terminal` 工具新增 `shell` 参数（powershell/msys2/gitbash/wsl），模型只在用户要求时临时切换后端；设置页 `defaultShell` 仍为用户默认；
> 3. **新增 `wslDistro` 配置**：WSL 默认发行版（未指定时用系统默认）。
>
> 完整差异与验证见 Marisa 仓库 `docs/plugins/dsh-bash-terminal.md`；上游反馈待 GitHub 可达后提交。

DSH（DeepSeek Harness）插件：一个 `shell` 工具，在 Windows 上统一执行 **PowerShell / MSYS2 / Git Bash / WSL** 四种终端命令。

| 后端 | 实际执行 | 语法 / 路径 | 环境变量 |
|------|----------|-------------|----------|
| `powershell`（默认） | `pwsh -NoLogo -NoProfile -NonInteractive -Command <cmd>` | PowerShell；`C:\...` | `$env:NAME` |
| `msys2` | MSYS2 根 `bash -lc <cmd>`（`C:\msys64` 等自动探测） | POSIX；`C:\...` 与 `/c/...` 皆可；pacman/mingw 可用 | `$NAME`（注入 `MSYSTEM`） |
...
- **终端默认由用户决定，模型仅在用户要求时切换**：Web UI 设置页（设置 → 通用）出现"默认终端"下拉（PowerShell / MSYS2 / Git Bash / WSL）；`shell` 工具默认只使用该设置。用户明确要求换 shell 时（如"这条命令用 WSL 跑"），模型可传 `shell` 参数临时切换，不改变设置默认值。设置通过 DSH settings 系统持久化（settings.yaml）。
