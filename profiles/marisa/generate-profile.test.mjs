...
test('generated Marisa profile mounts the complete vendored MyGO rc7 market stack', () => {
  ...
  try {
    ...
    for (const name of MYGO_PACKAGES) {
      assert.match(profile.dependencies[name], /^file:/, `${name} must be a vendored file dependency`);
    ...
    assert.match(bundle.dependencies['@deepseek-ai/cordis'], /^file:/, 'production bundle must retain Cordis for tool-cordis runtime imports');
    ...
    assert.doesNotMatch(standaloneOverlay, /id: ui-slash[\s\S]*disabled: true/);
    assert.doesNotMatch(standaloneOverlay, /id: ui-command[\s\S]*disabled: true/);
