/**
 * Slash pipeline spec over the split architecture. SlashService keeps only
...
interface PendingFetch {
  resolve: (items: readonly SlashCandidate[]) => void
...
function deferredSource(trigger: TriggerChar, name: string, over: Partial<SlashSource> = {}) {
  const pending: PendingFetch[] = []
  const warm = vi.fn()
  const source: SlashSource = {
    trigger,
    name,
    candidates: (session, req) => new Promise<readonly SlashCandidate[]>((resolve, reject) => {
...
function readySource(
  trigger: TriggerChar, name: string, items: readonly SlashCandidate[], onPick?: (pick: SlashPick) => PickOutcome,
) {
...
function controllerBench(sources: SlashSource[] = [], key = 'a') {
  ...
  const controller = new SlashController({ actx: scope.ctx, sessionId: sid(key), roster })
...
async function serviceBench() {
  ...
  await root.plugin(SlashService).await()
  const slash = root.get('slash') as SlashService
  const mint = (key: string) => {
    const scope = createScope(root, sid(key))
    return { actx: scope.ctx, fiber: scope.fiber }
  }
  return { root, slash, mint }
}

describe('registerSource', () => {
  it('throws on a duplicate (trigger, name); same name across triggers is fine', async () => {
    const { slash } = await serviceBench()
    slash.registerSource(readySource('/', 'command', []).source)
    expect(() => slash.registerSource(readySource('/', 'command', []).source))
  ...
  it('disposal frees the name and drops the live menu group in every session controller', async () => {
    ...
    slash.registerSource(a.source)
    const disposeB = slash.registerSource(b.source)
  ...
  it('HMR shape: dispose of the registering fiber removes the source', async () => {
    ...
    const fiber = root.plugin({
      apply(pluginCtx: Context) {
        pluginCtx.effect(
          () => slash.registerSource(readySource('/', 'command', [{ name: 'goal' }]).source),
          'test: slash source',
        )
      },
    })
...
describe('sessionOf', () => {
  it('resolves lazily: same scope → same resident controller; another session → its own', async () => {
    const { slash, mint } = await serviceBench()
    const a = mint('a')
    const first = slash.sessionOf(a.actx)
    expect(slash.sessionOf(a.actx)).toBe(first)
    expect(slash.sessionOf(mint('b').actx)).not.toBe(first)
  })

  it('throws off an unscoped context', async () => {
    const { root, slash } = await serviceBench()
    expect(() => slash.sessionOf(root)).toThrow(/requires a session scope/)
  })
  ...
  it('warms the roster once at controller birth with the session projection', async () => {
    ...
    slash.registerSource(cmd.source)
    slash.registerSource(sub.source)
    const a = mint('a')
    slash.sessionOf(a.actx)
  ...
  it('the scope disposer removes and disposes the controller; a re-mint resolves fresh', async () => {
    ...
    slash.registerSource(readySource('/', 'command', [{ name: 'goal' }]).source)
    const a = mint('a')
    const controller = slash.sessionOf(a.actx)
    controller.track('/g', 2, { tier: 'plain' }, 1)
    await tick()
    expect(controller.menu.getSnapshot().open).toBe(true)
...
describe('pick / scoped input events', () => {
  function pickBench(outcomeOf: (pick: SlashPick) => PickOutcome) {
...
describe('onSpace', () => {
  function spaceSource(name: string, answer: PickOutcome, calls: string[]): SlashSource {
    return {
      trigger: '/',
      name,
      candidates: () => Promise.resolve([]),
...
describe('adjudicate', () => {
  const enterSource = (
    ...
    matchEnter?: SlashSource['matchEnter'],
  ): SlashSource => ({
    trigger,
    name,
    candidates: () => Promise.resolve([]),
