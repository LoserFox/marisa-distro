...
/** Create a resolvable client package whose client export points at the returned path. */
function writePackage(packageName: string, metadata: 'legacy' | 'current' = 'legacy'): string {
...
describe('client bundle activation', () => {
  it('accepts the current dsh.client package metadata used by MyGO rc6', () => {
    const packageName = '@fixture/current-client-metadata'
    const clientPath = writePackage(packageName, 'current')
    mkdirSync(dirname(clientPath), { recursive: true })
    writeFileSync(clientPath, 'module.exports = {}\n')

    expect(construct([packageName]).graph().entries.map(entry => entry.id)).toContain(packageName)
  })
