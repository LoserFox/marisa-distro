...
`, { 'yarn.lock': '' })
    ...
    expect(() => { return SdkProject.create(defaults.root, { ...request(), features: [] }) }).toThrow('requires one app')
...
`)
  ...
  it('guards internal resource collisions and malformed aggregate documents', async () => {
    ...
    expect(() => { return project.readEnvironment('.env', 'KEY') }).toThrow('not an environment document')
