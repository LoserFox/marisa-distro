...
function pluginId(packageName: string): string {
  const base = !packageName.startsWith('@') ? packageName : packageName.slice(packageName.indexOf('/') + 1)
...
export async function runCreatePluginCommand(
  ...
  const add = context.add ?? ((manager, source, cwd) => { return manager.add(source, cwd) })
