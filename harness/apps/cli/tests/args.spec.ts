...
const parse = (argv: string[]) => { return parseDshArgs(argv, '1.2.3') }
...
describe('parseDshArgs', () => {
  it('routes profile boots, one-shot runs, and the web alias', () => {
    ...
    expect(parse(['web'])).toEqual({ mode: 'web', profile: 'web', dev: false, patches: [] })
    expect(parse(['web', '--patch', 'web.yml'])).toEqual({ mode: 'web', profile: 'web', dev: false, patches: ['web.yml'] })
    expect(parse(['web', '--host', '0.0.0.0', '--port', '8080', '--dev', '--workspace-root', '/w']))
      .toEqual({ mode: 'web', profile: 'web', host: '0.0.0.0', port: 8080, dev: true, workspaceRoot: '/w', patches: [] })
    expect(parse(['web', '--trusted-host', 'harness.internal:3080', 'lab.internal', '--trusted-host', '10.0.0.9']))
      .toEqual({ mode: 'web', profile: 'web', dev: false, patches: [], trustedHosts: ['harness.internal:3080', 'lab.internal', '10.0.0.9'] })
    expect(parse(['web', '--profile', 'tui', '--port', '8080']))
      .toEqual({ mode: 'web', profile: 'tui', port: 8080, dev: false, patches: [] })
  })
  ...
  it('routes profile and web config dumps', () => {
    ...
    expect(parse(['web', '--dump-default-config']))
      .toEqual({ mode: 'dump-config', profile: 'web', defaultOnly: true, patches: [] })
    expect(parse(['web', '--profile', 'tui', '--dump-config']))
