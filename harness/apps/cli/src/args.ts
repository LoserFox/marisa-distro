 ...
 * profile; `web` serves a named profile's web surface (default `web`) with the
 * Web flag family; `plugin` manages a profile's plugin dependencies by forwarding
...
/**
 * Browser UI: `dsh web` serves the named profile's web surface (default
 * `web`). Host and port remain unvalidated pass-throughs to the webserver
 * schema; absent values leave the shipped web bundle values intact.
 */
interface WebInvocation {
  mode: 'web'
  /** The profile under $DSH_HOME/profiles whose web surface to serve. */
  profile: string
  patches: string[]
  host?: string
  port?: number
...
Examples:
  ...
  dsh web --port 8080                        serve the default web profile with its flag family
  dsh web --profile tui --port 8080          serve a named profile's web surface
`)
    .exitOverride()
  ...
  const web = program.command('web').description('serve a named profile\'s browser UI (default profile: web) on the configured host and port')
  web
    .option('--profile <name>', 'profile under $DSH_HOME/profiles to serve; default web', 'web')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
    ...
    .option('--dump-config', 'print the composed profile tree (with the user layer and any --patch) and exit')
    .option('--dump-default-config', 'print the selected profile\'s bundle layers (no user layer) and exit')
