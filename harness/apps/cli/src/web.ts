/**
 * `dsh web` — the browser surface over a named profile boot (default
 * `web`) plus the Web flag family (`--host/--port/--dev/--workspace-root/
...
function deriveWebFlagPatches(
  ...
  const patches = [...overrides.entries()].map(([id, bag]): PatchOptions => {
    ...
    if (composed === undefined) throw new Error(`dsh: patch target row "${id}" not found in the ${flags.profile} profile composition`)
