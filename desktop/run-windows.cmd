@echo off
rem Marisa DSH Windows 开发启动器：启动 release\dsh-shell.exe（Wails 壳）。
...
rem 默认 0 由 OS 分配）。
setlocal
set "APP=%~dp0..\release\dsh-shell.exe"
if exist "%APP%" (
  start "" "%APP%"
) else (
  echo error: %APP% not found - run: pwsh -NoProfile -File build.ps1
  exit /b 1
)
